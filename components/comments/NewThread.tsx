...
export const NewThread = ({ children }: Props) => {
  ...
  const [allowUseComposer, setAllowUseComposer] =
    useState(false);
  ...
  const handleComposerSubmit = useCallback(
    (
      ...
      const { top, left } =
        overlayPanel.getBoundingClientRect();
  ...
  return (
    <>
      ...
      <Slot
        ...
        style={{
          opacity:
            creatingCommentState !== "complete" ? 0.7 : 1,
        }}
      >
        {children}
      </Slot>
      ...
      creatingCommentState === "placed" ? (
        ...
        <Portal.Root
          ...
          style={{
            pointerEvents: allowUseComposer
              ? "initial"
              : "none",
      ...
      <NewThreadCursor
        display={creatingCommentState === "placing"}
      />
    </>
  );
};
